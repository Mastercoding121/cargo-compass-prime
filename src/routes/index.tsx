...
import { Plane, Ship, Package, ShieldCheck, Globe2, Calculator, ShoppingBag } from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Procure from 1688.com, consolidate parcels and ship cargo from China to Nigeria. Instant ₦ NGN freight quotes." },
      { property: "og:title", content: `${BRAND.name} — ${BRAND.tagline}` },
      { property: "og:description", content: "Buy from 1688, ship to Lagos. Air, Sea & door delivery." },
    ],
  }),
  component: Landing,
});
    ...
    <div className="min-h-screen">
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pt-12 sm:pt-16 pb-10">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-5 text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tight leading-[1.05]">
            Buy from 1688, ship to <span className="text-primary">Nigeria</span> — without the friction.
          </h1>
          <p className="mt-5 text-muted-foreground max-w-xl">
            Pick products direct from 1688.com at ₦ NGN converted prices. We procure, consolidate and ship by air or sea straight to Lagos.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-6 text-xs text-muted-foreground">
            <span className="flex items-center gap-2"><Globe2 className="size-4 text-primary" /> Direct from China</span>
            <span className="flex items-center gap-2"><Ship className="size-4 text-primary" /> Sea & air freight</span>
            <span className="flex items-center gap-2"><Package className="size-4 text-primary" /> Lagos door delivery</span>
          </div>
        </div>
      </section>
        ...
        <Card className="border-primary/25 shadow-[var(--shadow-card)]">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Calculator className="size-4 text-primary" /> Try our freight calculators
            </CardTitle>
          </CardHeader>
          <CardContent>
            <FreightModalsRow />
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pb-16">
        <div className="flex items-end justify-between flex-wrap gap-3 mb-4">
          <div>
            <h2 className="text-2xl font-semibold tracking-tight">Live 1688 product feed</h2>
            <p className="text-sm text-muted-foreground">Procurement-ready inventory from verified Chinese suppliers.</p>
          </div>
        </div>
        <ProductGrid />
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pb-16 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          ...
          { icon: Plane, title: "Air & sea options", desc: "Lagos arrival in 14, 21 or 65 days depending on cargo class." },
        ].map((f) => (
          <Card key={f.title}>
            <CardHeader>
